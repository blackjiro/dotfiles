...
2. **レビュー実行（すべて必須）**
   ...
   - [ ] `codex review` を実行（以下のルールで引数を決定）:
     - **必須オプション**: `--sandbox read-only -a never` を常に付与（ファイル変更を防止）
     - 未コミット変更がある場合: `codex review --sandbox read-only -a never --uncommitted`
     - 未コミット変更がない場合: `codex review --sandbox read-only -a never --base <デフォルトブランチ>`
